 ...
 * @return string
 */
function adace_get_after_paragraph_2_slot_id() {
	return 'adace-after-paragraph-2';
}
...
adace_register_ad_slot(
	array(
		'id'             => adace_get_after_paragraph_2_slot_id(),
...
add_filter( 'adace_options_slot_fields_filter', 'adace_after_paragraph_2_slot_option', 10, 2 );
/**
 ...
 * @return array
 */
function adace_after_paragraph_2_slot_option( $slot_fields, $adace_ad_slot ) {
	if ( adace_get_after_paragraph_2_slot_id() !== $adace_ad_slot['id'] ) {
		return $slot_fields;
	}
...
add_action( 'adace_options_slots_field_renderer_action', 'adace_after_paragraph_2_slot_option_renderer', 10, 2 );
/**
 ...
 */
function adace_after_paragraph_2_slot_option_renderer( $args, $slot_options ) {
	if ( adace_get_after_paragraph_2_slot_id() !== $args['slot']['id'] ) {
		return;
	}
...
add_filter( 'adace_slots_options_save_validator_filter', 'adace_after_paragraph_2_slot_option_save_validator', 10, 2 );
/**
 ...
 */
function adace_after_paragraph_2_slot_option_save_validator( $input_sanitized, $input ) {
...
add_filter( 'adace_pre_query_slot', 'adace_after_paragraph_2_add_wrap_to_query',10,2 );
/**
 ...
 * @return array
 */
function adace_after_paragraph_2_add_wrap_to_query( $result, $slot_id ) {
	if ( adace_get_after_paragraph_2_slot_id() !== $slot_id ) {
		return $result;
	}
	$adace_ad_slots = adace_access_ad_slots();
	$slot_register = $adace_ad_slots[ adace_get_after_paragraph_2_slot_id() ];
	$slot_options = get_option( 'adace_slot_' . adace_get_after_paragraph_2_slot_id() . '_options' );
...
add_filter( 'the_content', 'adace_after_paragraph_2_slot_inject', 9999 );
/**
 ...
 * @return string
 */
function adace_after_paragraph_2_slot_inject( $content ) {
	if ( ! is_singular( ) ) {
		return $content;
	}
	if ( false === adace_is_ad_slot( adace_get_after_paragraph_2_slot_id() ) ) { return $content; }
	...
	if ( count( $paragraphs ) >= $inject_after ) {
		foreach ( $paragraphs as $paragraph_index => $paragraph_content ) {
			...
			if ( $inject_after === $paragraph_index + 1 ) {
				$paragraphs[ $paragraph_index ] .= adace_get_ad_slot( adace_get_after_paragraph_2_slot_id() );
			}
		}
		return implode( '', $paragraphs );
	} else {
		if ( $to_end_of_post ) {
			$content .= adace_get_ad_slot( adace_get_after_paragraph_2_slot_id() );
		}
	}
